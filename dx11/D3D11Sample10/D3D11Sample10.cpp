...
const std::wstring Application::m_title { L"Direct3D 11 Sample10" };
const std::wstring Application::m_windowClass { L"D3D11D10" };
...
HRESULT Application::initializeDirect3D()
{
	...
	result = readFile(L"D3D11Sample10Vs.cso", vsBlob);
	if (FAILED(result)) {
		return result;
	}
	...
	result = readFile(L"D3D11Sample10Gs.cso", gsBlob);
	if (FAILED(result)) {
		return result;
	}
	...
	result = readFile(L"D3D11Sample10Ps.cso", psBlob);
	if (FAILED(result)) {
		return result;
	}
	...
	result = m_device->CreateTexture2D(&textureDesc, nullptr, &texture);
	if (FAILED(result)) {
	...
	result = m_device->CreateShaderResourceView(texture.Get(), &shaderResourceViewDesc, &m_textureShaderResourceView);
	if (FAILED(result)) {
	...
	result = m_immediateContext->Map(texture.Get(), 0, D3D11_MAP_WRITE_DISCARD, 0, &mappedResource);
	if (FAILED(result)) {
	...
	m_immediateContext->Unmap(texture.Get(), 0);
