...
private:
	...
	CWavefrontObj m_wavefrontObj;
	CWavefrontMtl m_wavefrontMtl;
...
const std::wstring Application::m_title{ L"Direct3D 11 Sample13" };
const std::wstring Application::m_windowClass{ L"D3D11D13" };
...
HRESULT Application::initializeDirect3D()
{
	...
	result = readFile(L"D3D11Sample13Vs.cso", vsBlob);
	if (FAILED(result)) {
		return result;
	}
	...
	D3D11_INPUT_ELEMENT_DESC layout[] = {
		...
		{ "NORMAL",   0, DXGI_FORMAT_R32G32B32A32_FLOAT, 0, sizeof(DirectX::XMFLOAT3),     D3D11_INPUT_PER_VERTEX_DATA, 0 },
		{ "TEXTURE",  0, DXGI_FORMAT_R32G32_FLOAT,       0, sizeof(DirectX::XMFLOAT3) * 2, D3D11_INPUT_PER_VERTEX_DATA, 0 }
	};
	...
	result = readFile(L"D3D11Sample13Ps.cso", psBlob);
	if (FAILED(result)) {
		return result;
	}
	...
	// load model
	char materialFilename[80];
	if (!m_wavefrontObj.Load(m_device.Get(), m_immediateContext.Get(), "..\\misc\\cube.obj", materialFilename, sizeof(materialFilename))) {
		return TRACE_ERROR(-1, L"CWavefrontObj::Load() failed.");
	}
	if (!m_wavefrontMtl.Load(m_device.Get(), materialFilename, "..\\misc\\default.bmp")) {
		return TRACE_ERROR(-1, L"CWavefrontMtl::Load() failed.");
	}
...
void Application::cleanupDirect3D()
{
	...
	m_wavefrontMtl.Cleanup();
	m_wavefrontObj.Cleanup();
}
...
HRESULT Application::render()
{
	...
	DirectX::XMFLOAT3 center = m_wavefrontObj.GetBoundingSphereCenter();
	DirectX::XMMATRIX matTrans = DirectX::XMMatrixTranslation(-center.x, -center.y, -center.z);

	FLOAT scale = 1.0f / m_wavefrontObj.GetBoundingSphereRadius();
	DirectX::XMMATRIX matScale = DirectX::XMMatrixScaling(scale, scale, scale);

	FLOAT rotate = static_cast<FLOAT>(DirectX::XM_PI * (timeGetTime() % 3000)) / 1500.0f;
	DirectX::XMMATRIX matY = DirectX::XMMatrixRotationY(rotate);
	rotate = static_cast<FLOAT>(DirectX::XM_PI * (timeGetTime() % 1500)) / 750.0f;
	DirectX::XMMATRIX matX = DirectX::XMMatrixRotationX(rotate);
	DirectX::XMStoreFloat4x4(&m_constantBufferData.world, DirectX::XMMatrixTranspose(matTrans * matScale * matY * matX));

	m_wavefrontObj.SetIA();
	...
	WFOBJ_GROUP *group = m_wavefrontObj.GetGroup();
	int groupCount = m_wavefrontObj.GetCountGroup();
	for (int i = 0; i < groupCount; i++) {
		if (group[i].countIndex < 1) {
			continue;
		}

		WFOBJ_MTL *material = m_wavefrontMtl.GetMaterial(group[i].mtl);
		ID3D11ShaderResourceView *shaderResourceView = m_wavefrontMtl.GetTextureDefault(material->map_Kd);
		m_constantBufferData.diffuse.x = material->Kd[0];
		m_constantBufferData.diffuse.y = material->Kd[1];
		m_constantBufferData.diffuse.z = material->Kd[2];
		m_constantBufferData.diffuse.w = material->d;
		...
		m_immediateContext->PSSetShaderResources(0, 1, &shaderResourceView);

		m_wavefrontObj.Draw(i);
	}
